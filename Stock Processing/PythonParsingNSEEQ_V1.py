...
if WithFO=="Yes" :
    ...
    FOBaseFileData.drop(FOBaseFileData.columns.difference(['SYMBOL','Year','FOMode']), 1, inplace=True)
    FOBaseFileData['Year']=FOBaseFileData['Year'].astype(str)
    EQData['Year']=EQData['Year'].astype(str)
    EQData = pd.merge(EQData, FOBaseFileData, how='inner', left_on=['SYMBOL','Year'], right_on = ['SYMBOL','Year']) #use how='left' for left join
