...
SP500BaseFileData=pd.read_html('https://en.wikipedia.org/wiki/List_of_S%26P_500_companies')[0]
SP500BaseFileData["SYMBOL"] = SP500BaseFileData["Symbol"]
SP500BaseFileData["SECTOR"] = SP500BaseFileData["GICS Sector"]
SP500BaseFileData["INDUSTRY"] = SP500BaseFileData["GICS Sub-Industry"]
SP500BaseFileData =SP500BaseFileData[['SYMBOL','SECTOR','INDUSTRY']]
combined_csv = pd.merge(combined_csv, SP500BaseFileData, how='left', left_on=['SYMBOL'], right_on = ['SYMBOL']) #use how='left' for left join, inner for overlap
...
EQData['VOLATILITY_LOW']=EQData.groupby('SYMBOL',as_index=False,group_keys=False).apply(get_rolling,1500,'VOLATILITY','min')
...
for i in EQDataYear[1:]:
    EQData[EQData.Year == i].to_csv(path+i+"sp500_combined_csv_processed_output.csv", index=False, encoding='utf-8-sig')
...
print("pivot export")
EQBullish.to_csv(path+"sp500_combined_csv_processed_output_pivort_week.csv", encoding='utf-8-sig')
EQBullishYear.to_csv(path+"sp500_combined_csv_processed_output_pivort_Year.csv", encoding='utf-8-sig')
EQBullishMonth.to_csv(path+"sp500_combined_csv_processed_output_pivort_Month.csv", encoding='utf-8-sig')
EQBullishClose.to_csv(path+"sp500_combined_csv_processed_output_close_pivort_week.csv", encoding='utf-8-sig')
EQBullishCloseYear.to_csv(path+"sp500_combined_csv_processed_output_close_pivort_Year.csv", encoding='utf-8-sig')
EQBullishCloseMonth.to_csv(path+"sp500_combined_csv_processed_output_close_pivort_Month.csv", encoding='utf-8-sig')
